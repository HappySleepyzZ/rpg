...
- P1 Review 事项尚未自动处理：
  - 老的 `.uasset/.umap` 曾进入 Git 普通 blob；用户确认这是个人新仓库、没有协作者后，已执行 Git LFS 历史迁移。
  - 迁移命令：
  ...
  - 迁移后 3 个既有提交 hash 被重写；当前主线提交为：
    - `634ed1a fix: address prototype review findings`
    - `46aeb46 feat: add root motion dash prototype`
    - `04be449 初始提交：项目骨架、文档和基础角色代码`
  - `git lfs ls-files` 数量为 223，和当前已跟踪的 UE/媒体二进制资源数量一致。
  - `git lfs fsck` 已通过。
  - 迁移后曾出现工作区资产仍是 LFS pointer 文本，UE 报 `Invalid value for PACKAGE_FILE_TAG`；已用 `git lfs checkout` 恢复真实二进制资产，再用 `git add -u` 归一化索引，工作区恢复干净。
  - 迁移后再次运行 `Scripts/verify_core_assets.py`，UE 命令行验证通过，`Success - 0 error(s)`。
  - 因历史已重写，后续推送远端需要 force push。
  - `Content/Mannequins` 与 `Content/Characters/Mannequins` 重复资源已在用户确认后处理：保留 `Content/Characters/Mannequins`，删除 `Content/Mannequins`。
  ...
  - 新增 `Scripts/verify_core_assets.py` 用于加载核心玩家资产、Manny Mesh、`ABP_Unarmed`、`MM_Dash` 和 RootMotion Roll；删除重复资源后 UE 命令行验证通过，`Success - 0 error(s)`。
